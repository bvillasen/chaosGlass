...
cudaCodeFile = open("CUDAmandelbrot.cu","r")
...
mappingLogisticKernel = cudaCode.get_function('mandelbrot_kernel')
...
graphPoints_d= gpuarray.to_gpu( np.zeros([nHeight,nWidth], dtype=npPrcsn) ) 	
#For plotting
...
def replot():
  ...
  start.record()
  #random_d = curandom.rand((nData), dtype=npPrcsn)
  mappingLogisticKernel( npPrcsn(xMin), npPrcsn(xMax), npPrcsn(yMin), npPrcsn(yMax),np.int32( L ), graphPoints_d, grid=grid2D, block=block2D )
...
def normalize( data ):
  ...
  minVal = gpuarray.min(data).get()
  linearDouble(1./(maxVal-minVal), -minVal, data, data )
