...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
__global__ void mapping_kernel( const int nWidth, const int nHeight, cudaP xMin, cudaP xMax, cudaP yMin, cudaP yMax,cudaP *startingPoints, cudaP *graphPoints ){
  int t_x = blockIdx.x*blockDim.x + threadIdx.x;
  int t_y = blockIdx.y*blockDim.y + threadIdx.y;
  int tid = t_x + t_y*blockDim.x*gridDim.x;
  
  __shared__ unsigned char mappedPoints[ %(HEIGHT)s ];
  mappedPoints[t_y] = 0;
  __syncthreads();
  
  cudaP val = startingPoints[tid];
  cudaP k = (xMax - xMin)/(nWidth-1)*t_x + xMin;
  int nIterations = 100000;
  int nValues = 100;
  cudaP yFactor = nHeight/(yMax-yMin);
  int yPix;
